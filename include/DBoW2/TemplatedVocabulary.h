...
public:
    ...
    string wordsTableName = "words";
    string nodestableName = "nodes";
    string m_treeName;
  ...
  /**
   * Creates the vocabulary by loading from a pg database from table named by 
   * [vocabTableName] variable.
   * @param conn PgSql connection
   * @param treeName name of the tree to load
   */
  TemplatedVocabulary(pqxx::connection_base& conn, 
  ...
  /**
   * Saves vocabulary into PostgreSQL database
   * @param conn postgres connection
   */
  virtual void saveToPG(pqxx::connection_base &conn) const;
  ...
  /**
   * Loads te vocabulary from a PostgreSQL db
   * @param conn postgres connection
   */
  virtual void loadFromPG(pqxx::connection_base &conn);
...
TemplatedVocabulary<TDescriptor,F>::TemplatedVocabulary
  (const std::string &treeName, int k, int L, WeightingType weighting, ScoringType scoring)
  : m_k(k), m_L(L), m_weighting(weighting), m_scoring(scoring),
  m_scoring_object(NULL), m_treeName(treeName)
{
  createScoringObject();
}
...
template<class TDescriptor, class F>
TemplatedVocabulary<TDescriptor,F>::TemplatedVocabulary(pqxx::connection_base& conn, 
        const std::string& treeName) : m_scoring_object(NULL), 
        m_treeName(treeName)
{
    loadFromPG(conn);
}
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveToPG(pqxx::connection_base& conn) const
{
    try {
        string statementName = "checkStmt";
        work wCheck(conn, "CheckTables");
        conn.prepare(statementName, "select tablename from pg_tables "
        "where tablename = $1 or tablename = $2 or tablename = $3");
        result res = ((invocation)wCheck.prepared(statementName)
                (vocabTableName)(wordsTableName)(nodestableName)).exec();
        wCheck.abort();
        if (res.size() < 3)
        {
            createTables(conn);
        ...
        while(!parents.empty())
        {
          ...
          for(pit = children.begin(); pit != children.end(); pit++)
          {
            ...
            // save node data
            int nodeId = (int)child.id;
            int parentId = (int)pid;
            double weight = (double)child.weight;
            string descriptor = F::toString(child.descriptor);

            ((invocation)w.prepared(statementName)
                    (nodeId)(parentId)(weight)(descriptor)(m_treeName)).exec();
        ...
        for(wit = m_words.begin(); wit != m_words.end(); wit++)
        {
          ...
          int nodeId = (int)(*wit)->id;
          ((invocation)w.prepared(statementName)
                ((int)id)(nodeId)(m_treeName)).exec();
        }
        
        
        w.commit();
    } catch (const pqxx::pqxx_exception &ex) {
        cout << "Error while saving VocTree data into DB, method: saveToPG(): " << ex.base().what() << endl;
       
    }
}

template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::createTables(pqxx::connection_base& conn)  const
{
    try {
        string stmt = "voc";
        ...
        {
            std::cerr << "Error on creating table " << vocabTableName << ":\n" << e.base().what() << std::endl;
        }
        ...
        try {
            ...
            std::cerr << "Error on creating table " << nodestableName << ":\n" << e.base().what() << std::endl;
        }
        ...
        try {
            ...
            std::cerr << "Error on creating table " << wordsTableName << ":\n" << e.base().what() << std::endl;
        }
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::loadFromPG(pqxx::connection_base &conn)
{
    ...
    if (res.size() == 0)
    {
        throw pqxx::plpgsql_no_data_found("tree with name '" + m_treeName + 
                "' not found");
    }
    ...
    for (result::const_iterator row = res.begin(); row != res.end(); ++row)
    {
        ...
        break; //just in case. Need first row only. Normally it should be the one in result set.
    }
    res.clear();
    ...
    if (res2.size() == 0)
    {
        throw pqxx::plpgsql_no_data_found("Nodes with tree name '" + m_treeName + 
                "' not found");
    }
    
    m_nodes.resize(res2.size() + 1); // +1 to include root
    m_nodes[0].id = 0;
    ...
    WordValue weight;
    string d;
    for (result::const_iterator row = res2.begin(); row != res2.end(); ++row)
    {
        for (result::tuple::const_iterator column = row.begin(); column != row.end(); ++column)
        {                             
    ...
    if (res3.size() == 0)
    {
        throw pqxx::plpgsql_no_data_found("Words with tree name '" + m_treeName + 
                "' not found");
    }
    
    m_words.resize(res3.size());
    NodeId wid;
    NodeId _nid;
    for (result::const_iterator row = res3.begin(); row != res3.end(); ++row)
    {
        for (result::tuple::const_iterator column = row.begin(); column != row.end(); ++column)
        {                             
            if (strcmp(column.name(), "id") == 0) {
        ...
        m_nodes[_nid].word_id = wid;
        m_words[wid] = &m_nodes[_nid];
    }
    res3.clear();
